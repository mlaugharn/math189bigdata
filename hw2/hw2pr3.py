...
def linreg(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	# in our homework, we solved ||Ax - b||^2 + ||Lx||^2
	# we found x* = (A.T A + L)^-1 (A.T + b)
	# hence we are solving (A.T A + L) x* = (A.T + b)
	# here we have A = X, b = y, L = lambda^.5 * I
	# and w_opt = x*
	L = np.eye(X.shape[1]) * reg
	L[0, 0] = 0 # set bias term to 0 regulation
	W_opt = np.linalg.solve(X.T@X + L, X.T@y)
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def find_RMSE(W, X, y):
	...
	RMSE = np.sqrt(1/X.shape[0] * np.linalg.norm(y - X * W, 2)**2)
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	W_list = []
	"*** YOUR CODE HERE ***"
	reg_list = sorted(np.random.uniform(0.0, 150.0, 150))
	W_list = [linreg(X_train, y_train, reg) for reg in reg_list]
	RMSE_list = [find_RMSE(W_opt, X_val, y_val) for W_opt in W_list]
	#reg_list = np.array(reg_list).reshape(-1,1)
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	W_list = [linreg(X_train, y_train, reg) for reg in reg_list]
	#RMSE_list = [find_RMSE(W_opt, X_val, y_val) for W_opt in W_list	
	norm_list = [np.linalg.norm(W_opt, 2) for W_opt in W_list]
	"*** END YOUR CODE HERE ***"
...
def linreg_no_bias(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	# analytic solution says A^T(b - y) = (A^T A +  L^T L)x
	m = X.shape[0]
	I = np.eye(m)
	L = I * reg
	
	#_11t = np.ones(m) * (np.ones(m).T)
	middle = X.T @ (I - 1/m * np.ones(m))
	W_opt = np.linalg.solve(middle @ X + reg * np.eye(middle.shape[0]), middle @ y)
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	b_grad = 1.0
	#W_grad = 2/m_train *(X_train.T @ X_train * W + b*X_train.T * ones - X_train.T @ y_train + Lb * W)
	#b_grad = 2 * (ones * X_train * W - ones.T * y_train + b*m_train) / m_train
	"*** END YOUR CODE HERE ***"
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		W_grad = ((X_train.T @ X_train + Lb) @ W + X_train.T @ (b - y_train)) / m_train
