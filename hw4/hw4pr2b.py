...
def NLL(X, y, W, reg=0.0):
	"""	This function takes in four arguments:
			...
			2) y, the label of the data with dimension m x k
			3) W, a weight matrix
	...
	mu = X@W
	e_mu = np.exp(mu)
	denom = e_mu.sum(axis=1).reshape(-1,1)
	p = e_mu/denom
	# 
	return -(y * np.log(p)).sum(axis=1).sum() + reg*np.diag(W.T @ W).sum()
...
def grad_softmax(X, y, W, reg=0.0):
	...
	mu = X@W
	e_mu = np.exp(mu)
	p = e_mu / e_mu.sum(axis=1).reshape(-1,1)
	return X.T@(p - y) + reg*W
