...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	y_pred = sigmoid(X @ W)
	nll = -np.sum(np.multiply(y, np.log(y_pred)) + np.multiply(1 - y, np.log(1 - y_pred))) + reg / 2 * np.linalg.norm(W)**2
	nll = nll.item(0)
	"*** END YOUR CODE HERE ***"
	return nll
...
def get_description(X, y, W):
	...
	for i in range(X.shape[0]):
		y_i = y[i]
		y_pred_i = y_pred[i]
		if y_i == y_pred_i: accurate += 1
		if y_pred_i == 1:
			pred_1 += 1
			if y_i == 1: precise += 1
		if y_i == 1:
			true_1 += 1
			if y_pred_i == 1: recalled += 1
	accuracy = accurate / X.shape[0]
	precision = precise / pred_1
	recall = recalled / true_1
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	#reg_list = [0] + [3**i for i in range(-4, 4)]
	reg_list = [0.0, 0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 30.0]
	for reg in reg_list:
		W, nll_list = grad_descent(X_train, y_train, reg, lr=2e-4, print_freq=100)
		a, p, r, f1 = get_description(X_test, y_test, W)
		a_list.append(a)
		p_list.append(p)
		r_list.append(r)
		f1_list.append(f1)
