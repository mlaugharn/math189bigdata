...
try:
    ...
    print("scipy has removed ndimage.imread.. please pip install imageio")
...
if __name__ == '__main__':
        ...
        # NOTE: ndimage.imread was deprecated 
        img = image.imread('clown.jpg')
        grayimg = .299 * img[:, :, 0] + .587 * img[:, :, 1] + .114 * img[:, :, 2]
        img = grayimg
        ...
        Us, Ss, Vs = np.linalg.svd(shuffle_img)
        ...
        orig_S_plot = ax.plot(range(k), S[:k])
        shuf_S_plot = ax.plot(range(k), Ss[:k], color='g')
        "*** END YOUR CODE HERE ***"
        ...
        for index in range(len(rank_list)):
                ...
                truncated_s = S.copy()
                truncated_s[k:] = 0
                print(f"u.shape {U.shape} truncated_s.shape {truncated_s.shape} V.shape {V.shape}")
                imgk = (U[:, :k] * truncated_s[:k]) @ V[:k, :]
                plt.imshow(imgk, cmap='Greys_r')
                "*** END YOUR CODE HERE ***"
